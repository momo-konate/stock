...
export const resetPassword = async (req, res) => {
  try {
    ...
    if (!user.securityAnswer) {
      return res.status(400).json({ message: 'Aucune question de sécurité configurée pour ce compte' });
    }
...
export const getSecurityQuestion = async (req, res) => {
  try {
    ...
    if (!user.securityQuestion) {
      return res.status(400).json({ message: 'Aucune question de sécurité configurée' });
    }
